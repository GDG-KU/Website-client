...
export default function ModalEditPoint({ isOpen, onClose, user, onSave }: ModalEditPointProps) {
  ...
  return (
    <div className={styles["modal-edit-point-backdrop"]} onClick={onClose}>
      <div className={styles["modal-edit-point-content"]} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles["modal-body"]}>
          <div className={styles["modal-right"]}>
            <div className={styles["current-point"]}>
              현재 포인트 <strong>{user.roles.reduce((sum, r) => sum + r.point, 0)}</strong> P
            </div>
            ...
            <button className={styles["save-button"]} onClick={handleSave} disabled={loading}>
              {loading ? "저장 중..." : "저장하기"}
            </button>
          </div>
        </div>
