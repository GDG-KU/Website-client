...
export default function ActivityManagementPage() {
  ...
  const filteredData = useMemo(() => {
    let data = users;
    if (searchTerm.trim() !== "") {
      data = data.filter((u) => u.nickname.toLowerCase().includes(searchTerm.trim().toLowerCase()));
    }
    return data;
  }, [users, searchTerm]);
  ...
  const handleUserClick = (user: UserData) => setSelectedUser(user);
              ...
              <div className={styles.tabContent}>
                {activeTab === "fetch" && (
                  <div className={styles.activityPanel}>
                    {selectedUser.activities?.fetch?.map((item, idx) => <p key={idx}>{item}</p>)}
                  </div>
                )}
                {activeTab === "worktree" && (
                  <div className={styles.activityPanel}>
                    {selectedUser.activities?.worktree?.map((item, idx) => <p key={idx}>{item}</p>)}
                  </div>
                )}
                {activeTab === "branch" && (
                  <div className={styles.activityPanel}>
                    {selectedUser.activities?.branch?.map((item, idx) => <p key={idx}>{item}</p>)}
                  </div>
                )}
                {activeTab === "solution challenge" && (
                  <div className={styles.activityPanel}>
                    {selectedUser.activities?.solutionChallenge?.map((item, idx) => <p key={idx}>{item}</p>)}
                  </div>
                )}
              </div>
            </div>
