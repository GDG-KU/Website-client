...
export default function QualificationPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    return users.filter((u) => u.nickname.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [users, searchTerm]);
    ...
    <div className={styles.qualificationContainer}>
      ...
      <div className={styles.filterRow}>
        {roleFilters.map((r) => (
          <button
            key={r}
            className={`${styles.filterButton} ${filterRole === r ? styles.active : ""}`}
          ...
          <ul className={styles.memberList}>
            {filteredData.map((user) => (
              <li
                ...
                {user.nickname}
                <span className={styles.smallRole}>{user.position || "N/A"}</span>
              </li>
            ))}
          </ul>
        </div>
            ...
            <div className={styles.userDetail}>
              <div className={styles.userHeader}>
                ...
                {selectedUser.isAdmin ? (
                  <button className={styles.adminButtonRemove} onClick={handleRemoveAdminClick}>
                    관리자 삭제
                  </button>
                ) : (
                  <button className={styles.adminButtonAdd} onClick={handleAddAdminClick}>
                    관리자 추가
                  </button>
                )}
              </div>
      ...
      {showConfirmModal && selectedUser && (
        <div className={styles.modalBackdrop} onClick={() => setShowConfirmModal(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <h3>
              {selectedUser.nickname} 님을 관리자에 {confirmType === "ADD" ? "추가" : "삭제"} 하시겠습니까?
            </h3>
            <div className={styles.modalButtons}>
              <button className={styles.confirmButton} onClick={handleConfirmAdmin}>
